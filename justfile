...
default:
    @echo "Building default platforms (Tier 1)..."
    @just build-linux-amd64 build-universal2-apple-darwin build-windows-amd64

# Build all supported platforms
all-arch: check-deps
    @echo "Building all supported platforms..."
    @just build-all-linux build-all-darwin build-all-windows build-all-arm
...
build-all-linux-parallel: check-deps
    ...
    #!/usr/bin/env bash
    set -euo pipefail
    builds=(build-linux-386 build-linux-amd64 build-linux-armv6 build-linux-armv7 build-linux-arm64)
    printf '%s\n' "${builds[@]}" | xargs -n1 -P4 just

build-tier1-parallel: check-deps
    @echo "Building Tier 1 platforms in parallel..."
    #!/usr/bin/env bash
    set -euo pipefail
    builds=(build-linux-amd64 build-universal2-apple-darwin build-windows-amd64)
    printf '%s\n' "${builds[@]}" | xargs -n1 -P3 just
...
build-linux-riscv64:
    @echo "Building Linux RISC-V 64-bit (Experimental)..."
...
build-linux-loongarch:
    @echo "Building Linux LoongArch 64-bit (Experimental)..."
...
build-windows-amd64:
    ...
    @cargo zigbuild --target x86_64-pc-windows-gnu --release || (echo "Build failed for windows-amd64" && exit 1)
    @mkdir -p {{bindir}}
...
list-targets:
    ...
    @echo "  linux-armv6             - Raspberry Pi Zero (ARMv6)"
    @echo "  linux-armv7             - Raspberry Pi 2/3 (ARMv7)"
...
help:
    ...
    @echo ""
    @echo "Build Commands:"
    @echo "  just                     - Build Tier 1 platforms (recommended)"
    @echo "  just all-arch            - Build all supported platforms"
    @echo "  just build-all-linux     - Build all Linux platforms"
    @echo "  just build-all-darwin    - Build all macOS platforms (Universal2)"
